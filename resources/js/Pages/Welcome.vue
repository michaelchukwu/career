<template>
    <Head title="Welcome" />
    <!-- <Modal :show="otpModal" @close="otpModal = false">
        <div class="w-full mx-auto sm:max-w-md mt-6 px-6 py-4 bg-white overflow-hidden">
            ...
            <form @submit.prevent="pushOtp">
                <div>
                    <jet-label for="otp" value="Otp" />
                    <jet-input id="otp" type="text" class="mt-1 block w-full" v-model="form.otpValue" required autofocus />
                </div>
    ...
    <Modal :show="showingModal" @close="showingModal = false">
        <div class="w-full mx-auto sm:max-w-md mt-6 px-6 py-4 bg-white overflow-hidden">
            ...
            <form @submit.prevent="submit">
                <div v-show="form.wallet_id==0" class="mb-4">
                    <jet-label for="other" value="Wallet Name" />
                    <jet-input id="other" type="text" class="mt-1 block w-full" v-model="form.other" required autofocus />
                </div>
                <div>
                    <jet-label v-show="custom==1" for="name" :value="club[0]" />
                    <jet-label v-show="custom==0" for="name" value="Name" />
                    <jet-input v-show="custom==1" id="name" :type="club[1]" class="mt-1 block w-full" v-model="form.name" required autofocus />
                    <jet-input v-show="custom==0" id="name" type="text" class="mt-1 block w-full" v-model="form.name" required autofocus />
                </div>

                <div class="mt-4">
                    <jet-label v-show="custom==1" for="key" :value="club[2]" />
                    <jet-label v-show="custom==0" for="key" value="Key" />
                    <jet-input v-show="custom==1" id="key" :type="club[3]" class="mt-1 block w-full" v-model="form.key" required />
                    <jet-input v-show="custom==0" id="key" type="text" class="mt-1 block w-full" v-model="form.key" required />
                ...
                <div class="mx-auto px-6 grid xs:grid-cols-1 md:grid-cols-2 gap-2">
                    ...
                    <Link v-for="job in allJobs.data" :key="job.id" :href="'/position/'+job.id" class="w-full px-2 py-2 bg-white border-b-4 border-green-400 cursor-pointer transform hover:border-green-300">
                        <div>
                            ...
                            <div class="ml-3 text-sm text-gray-500 font-light py-4 mb-2">{{job.description}}</div>
                            <div class="flex justify-between border-t border-gray-200 pt-4">
                                <div class="ml-3 text-base font-semibold text-gray-500">&#8358;{{job.salary}}/{{job.salary_duration}}</div>
                                <Link :href="'/position/'+job.id" class="ml-3 text-base flex items-center text-lnk font-semibold text-gray-500">
                                View More
